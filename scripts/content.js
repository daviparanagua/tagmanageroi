...
chrome.runtime.onMessage.addListener( function(request, sender, sendResponse) {
    ...
    tr.innerHTML = '<td>' + new Date().toLocaleString('pt-BR',{ hour:'2-digit', minute: '2-digit' ,second: '2-digit' }) + '</td>' + '<td>' + parseField(data.ec) + '</td>' + '<td>' + parseField(data.ea) + '</td>' + '<td>' + parseField(data.el) + '</td>';
...
document.querySelector('#ga_oi_events_toggle_position').addEventListener('click', function(){
    document.querySelector('#gaoi_analyzer').classList.toggle('ga-analyzer-right');
})
